...
const App: React.FC = () => {
    ...
    const handleToolSelect = useCallback((tool: 'PAINT' | 'ERASER' | 'PICKER') => {
        setPopupInfo(null); // Close the popup when a tool is selected
        setActiveTool(prev => (prev === tool ? null : tool));
    }, []);

    const handlePlacePixel = useCallback((key: string) => {
    ...
    }, [paintCan, selectedColor, pixels, setPixels, setPaintCan, updateColorHistory]);

    const handleErasePixel = useCallback((key: string) => {
    ...
    const handlePickColor = useCallback((key: string) => {
    ...
    const toggleInfoModal = useCallback(() => setIsInfoModalOpen(prev => !prev), []);
    const toggleColorPicker = useCallback(() => setIsColorPickerOpen(prev => !prev), []);

    // --- Popup Handlers ---
    const handleShowPopup = useCallback((latlng: LatLng) => {
    ...
    const handleShareFromPopup = useCallback((latlng: LatLng) => {
        const shareZoom = Math.max(zoom, 18);
        const url = `${window.location.origin}${window.location.pathname}?lat=${latlng.lat.toFixed(6)}&lng=${latlng.lng.toFixed(6)}&zoom=${shareZoom}`;
